...
- **방송 발견용 폴링이나 숨은 대체 조회는 없다.** 알림 수신에 실패하면 상태와 로그에 표시한다. 수신 실패를 주기 조회로 우회하지 않는다. 예약 라이브(`upcoming`) 알림은 예외가 아니다 — YouTube가 같은 응답으로 알려 준 `scheduledStartTime` 까지는 요청을 하나도 보내지 않고, 그 시각부터만 그 영상 하나를 1분 간격으로 최대 30회 확인한 뒤 포기한다([#82](https://github.com/kor-haru/yt_rec/issues/82)).
...
8. 선택한 채널의 알림이 도착하면 영상 ID 한 건을 확인하고 라이브 녹화를 시작한다.
   **녹화 중** 카드의 크기와 경과 시간이 늘어나는지 확인한다. 녹화 대상으로 선택하지
   ...
   예약 라이브 알림(보통 시작 30분 전)이면 곧바로 녹화하지 않고 YouTube가 알려 준
   예정 시각까지 기다렸다가 시작을 확인한다. 이 예약은 디스크에 남아 앱을 껐다 켜도
   이어진다. 프리미어는 기본적으로 녹화하지 않으며 **설정 → 예약된 프리미어도 녹화**로
   켤 수 있다.
