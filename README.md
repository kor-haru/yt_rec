...
- **YouTube 알림은 전용 프로필 Chrome이 받는다.** 설치된 Chrome을 `%LOCALAPPDATA%\yt-rec\chrome-push` 프로필로 띄우고 CDP로 YouTube 서비스 워커에 붙어 알림을 넘겨받는다. 사용자의 개인 Chrome 프로필은 쓰지 않는다. 내장 QtWebEngine 수신기는 설정에서 고를 수 있게 남아 있지만, Google이 Chromium의 구형 GCM 등록 엔드포인트를 폐기해 현재는 푸시 구독을 새로 만들지 못한다([#62](https://github.com/kor-haru/yt_rec/issues/62), [#79](https://github.com/kor-haru/yt_rec/issues/79)). 구독 목록을 읽는 OAuth는 별도로 시스템 기본 브라우저와 로컬 루프백 서버를 사용한다. PySide6 6.11.1 이상(7 미만)이 필요하며 `uv sync`가 잠금 파일의 검증 버전을 설치한다.
- **방송 발견용 폴링이나 숨은 대체 조회는 없다.** 알림 수신에 실패하면 상태와 로그에 표시한다. 수신 실패를 주기 조회로 우회하지 않는다. 예약 라이브(`upcoming`) 알림은 예외가 아니다 — YouTube가 같은 응답으로 알려 준 `scheduledStartTime` 의 60분 전까지는 요청을 하나도 보내지 않는다. 방송자가 예고보다 일찍 켜므로 그 창이 열린 뒤에는 그 영상 하나만 1분 간격으로 확인하고([#103](https://github.com/kor-haru/yt_rec/issues/103)), 예정 시각 전 최대 60회·예정 시각 이후 최대 30회를 각각 따로 세어 다 쓰면 포기한다([#82](https://github.com/kor-haru/yt_rec/issues/82)).
...
창의 **X**, 작업 표시줄의 창 닫기, **앱 → 종료**, 트레이 메뉴 **종료**는 모두 종료 절차다.
녹화 중이라면 확인 창이 나오고, 받은 영상의 저장·병합이 끝날 때까지 안내가 표시된다.
창만 숨기고 수신·녹화를 유지하려면 **최소화**를 쓰고, 설정에서 **최소화하면 트레이로 보내기**를
켠다. 트레이 아이콘을 누르면 창이 다시 열린다.

**시작 시 창 숨김**을 켜도 트레이가 없으면 창이 보인다. 설정의 **로그인 시 자동 시작**은
다음 컴퓨터 로그인부터 적용된다. 자동 시작을 켠 뒤 실행 파일이나 소스 폴더를 옮겼다면
설정을 껐다가 다시 켠다.
...
uv 0.12.4 이상이면 빈 터미널 창이 따로 뜨지 않는다. 다만 명령을 친 터미널은 앱이
끝날 때까지 점유되므로, 앱이 실행되는 동안 함께 열어 둔다.

그보다 낮은 uv로 만든 `.venv`는 uv를 올려도 바뀌지 않는다. 그 `.venv`의 `pythonw.exe`는
이름과 달리 콘솔 프로그램이라, GUI 실행 파일을 그것으로 부르면 빈 터미널 창이 함께 뜬다. `uv self update`로 uv를 올린 뒤 프로젝트 폴더의
`.venv`를 지우고 `uv sync`를 다시 실행한다. uv가 0.12.4보다 낮으면 `uv sync`와 `uv run`이
버전이 맞지 않는다며 멈춘다.

**Windows에서 터미널 없이 띄우려면** 프로젝트 폴더의 **yt-rec.vbs**를 더블클릭한다.
같은 폴더의 `.venv\Scripts\yt-rec.exe`를 창 없이 실행할 뿐이고, 자동 시작도 이 런처를
쓴다. 터미널을 열어 둘 필요가 없고, uv 0.12.4 미만이 만든 `.venv`에서도 빈 터미널 창이
뜨지 않는다. `uv sync`를 아직 하지 않았다면 런처가 실행 파일을 찾지 못했다고 알린다.
실행 파일 배포판은 해당 없다.

빈 터미널 창 문제는 Windows 전용이다. macOS는 `~/Library/LaunchAgents`의 plist로,
리눅스는 `Terminal=false`인 `.desktop`으로 자동 시작하므로 터미널이 뜨지 않는다.
이 런처는 Windows 전용 편의일 뿐이고, 앱은 어느 운영체제에서도 이것에 의존하지 않는다.
...
8. 선택한 채널의 알림이 도착하면 영상 ID 한 건을 확인하고 라이브 녹화를 시작한다.
   **녹화 중** 카드의 크기와 경과 시간이 늘어나는지 확인한다. 녹화 대상으로 선택하지
   ...
   예약 라이브 알림(보통 시작 30분 전)이면 곧바로 녹화하지 않고 알림 받은 시점부터
   1분 간격으로 시작을 확인한다. 방송자가 예고보다 일찍 켜도 그때 붙는다. 이 예약은
   디스크에 남아 앱을 껐다 켜도 이어진다. 프리미어는 기본적으로 녹화하지 않으며
   **설정 → 예약된 프리미어도 녹화**로 켤 수 있다.
...
#### 알림 수신기: Chrome(기본)과 내장 브라우저

기본 수신기는 **설치된 Chrome**이다. 앱이 `%LOCALAPPDATA%\yt-rec\chrome-push` 전용 프로필로
Chrome을 띄워 두고, CDP(원격 디버깅)로 YouTube 서비스 워커에 붙어 알림을 넘겨받는다.
디버깅 통로는 루프백(127.0.0.1)에만 열리며 Origin도 그 주소만 허용한다. 사용자의 개인 Chrome
프로필(`Default`)은 읽지도 쓰지도 않는다. **YouTube 로그인**과 **YouTube 알림 설정**을 누르면
앱 안이 아니라 그 Chrome 창의 탭이 앞으로 나온다.

수신기는 주기적으로 방송을 조회하지 않는다. 30초마다 도는 것은 브라우저가 살아 있는지 묻는
확인뿐이고, YouTube에는 아무것도 묻지 않는다. Chrome이 꺼지면 **수신 브라우저 꺼짐**으로
드러내고 간격을 늘려 가며 다시 띄운다.

**설정 → 알림 수신기**에서 내장 QtWebEngine 수신기로 바꿀 수 있다(앱 재시작 필요). 다만 Google이
Chromium의 구형 GCM 등록 엔드포인트를 폐기해 내장 경로는 현재 **푸시 등록 없음**에서 더 나아가지
못한다. 외부 요인이 풀릴 때를 대비해 지우지 않고 남겨 둔 선택지다.
...
- **수신 브라우저 없음**: Chrome을 찾지 못했다. Chrome을 설치한 뒤 앱을 다시 실행한다.
- **수신 브라우저 꺼짐**: 수신용 Chrome이 종료되었거나 연결이 끊어졌다. 앱이 다시 띄우려
  시도하며, 이 표시가 남아 있으면 **로그**에서 실행 실패 원인을 확인한다.
...
**파일 위치 열기**, **경로 복사**로 파일을 사용할 수 있다. 다른 폴더로 옮기거나 삭제한 파일은
누락 상태로 표시되며, 앱을 다시 열어도 완료 이력은 남는다.

옮기거나 삭제한 영상의 이력이 필요 없다면 다음과 같이 정리할 수 있다.

1. **보관함 열기**를 누른 뒤 **새로고침**을 눌러 현재 파일 상태를 확인한다.
2. 한 건만 정리하려면 목록에서 해당 영상을 선택하고 **이력에서 제거**를 누른다.
   파일이 남아 있는 항목도 이 방법으로 보관함에서 숨길 수 있다.
3. 파일이 없는 항목을 한 번에 정리하려면 **파일 없는 이력 정리**를 누른다.
   검색 중이어도 전체 보관함이 대상이다. 확인 창에서 정리할 건수를 살펴본다.
4. 확인 창에서 **이력에서 제거**를 누른다. 마음이 바뀌면 **취소**를 누른다.
   창 아래에 제거한 건수가 표시되면 완료다. 저장 오류가 나오면 이력은 그대로 남는다.

이 기능은 **보관함 목록만 정리한다. 실제 영상이나 복구용 녹화 조각은 삭제하지 않는다.**
제거한 항목은 새로고침하거나 앱을 다시 실행해도 나타나지 않는다. 진행 중인 녹화와
자동 녹화의 중복 방지 기록에도 영향을 주지 않는다.

외장 드라이브가 빠져 있거나 폴더 접근 권한이 없으면 파일이 정말 없는지 알 수 없으므로
일괄 정리에서 제외한다. 드라이브를 다시 연결하거나 폴더 접근 문제를 해결한 다음
**새로고침**을 누른다. 파일 유무를 확인할 수 없어도 특정 이력을 숨기고 싶다면,
해당 항목을 직접 선택해 **이력에서 제거**를 사용한다.

이력을 한 번에 다 지우려면 메인 화면 `최근 완료` 의 **이력 비우기**를 누른다. 확인창에서
비울 건수를 보고 **이력 비우기**를 다시 누르면 보이는 완료 이력이 모두 사라진다. 기본
선택은 **취소**다. 이것도 **저장된 영상 파일은 삭제하지 않는다** — 목록에서만 사라진다.
비울 이력이 없으면 버튼은 눌리지 않고, 제외목록을 읽지 못한 상태에서는 기존 기록을
덮어쓰지 않으려고 비우기를 거절하며 사유를 상태 표시줄에 알린다.

### 완료 파일 삭제

실제 파일도 정리하려면 보관함을 새로고침하고 완료 또는 부분 복구 항목 하나를 선택한 뒤
**파일 삭제**를 누른다. 확인창의 방송 제목과 전체 경로를 확인하고 **휴지통으로 이동**을
누른다. 기본 선택은 **취소**다. 휴지통 이동 실패 시 앱은 영구 삭제로 재시도하지 않는다.

등록된 출력 폴더 바로 아래의 저장된 최종 미디어만 처리한다. 진행·병합 중인 녹화,
확인 후 바뀐 상태나 파일, 링크·정션·하드링크·디렉터리·작업 폴더 안의 파일은 거부한다.
녹화 원본 상태, 중간 조각, 자동 녹화 중복 방지 기록은 그대로 보존한다.
파일 이동 후 이력 저장만 실패하면 두 결과를 구분해 표시하며, 남은 항목에서
**이력에서 제거**를 눌러 저장을 다시 시도할 수 있다. 휴지통 이동만으로 디스크 공간이
즉시 확보되지는 않는다. 앱은 휴지통을 비우지 않는다.

네트워크 드라이브나 NAS에 저장된 파일은 Windows 휴지통이 없는 경우가 많다.
NAS에 Recycle/휴지통을 켜 두지 않았다면 **파일 삭제**는 거절되며 영상은 그대로 둔다.
앱은 그 경우 영구 삭제로 넘어가지 않는다. NAS에서 파일을 지우려면 파일 관리자에서
직접 지운 뒤 보관함에서 **이력에서 제거**를 누른다.

제외목록이 손상되면 저장된 출력 폴더의 이력은 계속 표시하지만, 기존 제외목록을
덮어쓰거나 파일을 삭제하지 않는다. 로그에 표시된 제외목록 오류를 해결한 뒤 앱을 다시 연다.
...
닫기(X)와 작업 표시줄의 창 닫기는 **앱 → 종료**와 같다. 녹화 중이면 확인 창이 나온다.
트레이로만 숨기려면 최소화 버튼을 쓴다. 트레이가 없는 환경에서도 닫기(X)는 종료다.
...
`uv run yt-rec`는 콘솔이 붙지 않는 GUI 런처다. uv 0.12.4 이상으로 만든 `.venv`에서는
앱을 띄울 때마다 빈 터미널 창이 따라붙지 않는다. 명령을 친 터미널은 앱이 끝날 때까지
점유된다. 그보다 낮은 uv로 만든 `.venv`는 [8. 실제 모드로 실행하기](#8-실제-모드로-실행하기)대로
다시 만든다. 표준출력으로 결과를 내는 `--help`와 `--smoke-test`는 같은
