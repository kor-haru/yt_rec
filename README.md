...
**Windows에서 터미널 창 없이 띄우려면** 프로젝트 폴더의 **yt-rec.vbs**를 더블클릭한다.
같은 폴더의 `.venv\Scripts\yt-rec.exe`를 창 없이 실행할 뿐이고, 자동 시작도 이 런처를
쓴다. 이 파일이 따로 있는 이유는 uv가 venv에 만드는 `pythonw.exe`가 이름과 달리 콘솔
프로그램이라서다 — GUI 실행 파일을 그것으로 부르면 빈 터미널 창이 함께 뜬다. `uv sync`를
아직 하지 않았다면 런처가 실행 파일을 찾지 못했다고 알린다. 실행 파일 배포판은 해당
없다.
