...
def test_naive_시각도_timestamp와_countdown이_같은_규칙을_쓴다() -> None:
    ...
    이 검사는 수정 전에도 통과한다(회귀 방지용). 원래 코드도 naive 값을 로컬로
    해석했기 때문이다. 다만 원래는 `지금의 UTC 오프셋`을 찍는 방식이어서 서머타임
    경계를 넘는 시각에서 한 시간이 어긋났고, 그 조건은 서머타임이 없는 시간대에서
    재현할 수 없다. 여기서 고정하는 것은 **naive 가 들어왔을 때의 동작이 정해져
    있고 두 함수가 같다**는 것이다.
    """
