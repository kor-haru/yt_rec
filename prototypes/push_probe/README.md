...
- 2026-09-12의 실제 시험 프로필에서는 로그인 유지, 알림 권한 `granted`,
  서비스 워커 존재를 확인했고 푸시 구독은 `false`였다. 2026-09-13에는 같은 프로필의
  정상 알림 설정 페이지에서 브라우저 알림이 이미 켜져 있었고, 한 번의 확인으로
  푸시 구독 `true`를 확인했다. 이때 스위치는 변경하지 않았다. 앞선 `false`와
  달라진 원인이 UI 수정이나 재시작이라고 단정할 수는 없다.
  실제 라이브 알림 수신과 YouTube의 데이터 형식은
  **아직 검증하지 않았다**. 구독 존재를 실제 알림 전달 증거로 취급하지 않는다.
...
4. 창 위쪽의 **알림 설정 열기**를 누른다. 같은 시험 프로필로 YouTube의
   [알림 설정](https://www.youtube.com/account_notifications)이 열린다.
   **데스크톱 알림 표시 → 이 브라우저에서 알림 받기 (Chrome)**가 꺼져 있으면 켠다.
   YouTube가 웹 알림을 요청하면 시험 창의 권한 안내에서 허용 여부를 선택한다.
   이 버튼은 설정 페이지로 이동할 뿐, 다른 계정 설정을 바꾸거나 푸시 구독을 임의로 만들지 않는다.
...
시험 프로그램을 수정한 뒤에는 **이 시험 창만** 닫고 종료된 것을 확인한 다음
위 실행 명령을 다시 실행한다. 같은 프로필을 동시에 여는 창을 두 개 만들지 않는다.
`.profile-youtube`를 삭제하거나 다른 브라우저의 프로필·쿠키를 복사하지 않는다.
이미 로그인되어 있으면 같은 시험 프로필을 그대로 사용한다.

알림 권한 허용이나 서비스 워커 존재만으로 푸시 구독·실제 수신이 증명되지는 않는다.
설정을 켠 뒤 확인 버튼을 한 번 눌러 구독 상태를 확인한다. 여전히 `false`이거나
등록 오류가 나타나면 원인 확인이 필요하며, 프로필 초기화나 반복 켜기/끄기로 우회하지 않는다.
