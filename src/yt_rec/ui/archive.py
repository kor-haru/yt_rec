...
from ..state.events import ArchiveDeleteFinished, ArchiveDismissFinished
...
class ArchiveDialog(QDialog):
    def __init__(self, state: AppState, parent: QWidget | None = None) -> None:
        ...
        self.delete_button = QPushButton("파일 삭제", self)
        self.delete_button.setToolTip("선택한 완료 파일 하나를 휴지통으로 옮긴 뒤 이력에서 제거합니다")
        ...
        cleanup.addWidget(self.delete_button)
        cleanup.addStretch()
        ...
        state.archive_delete_finished.connect(self._on_delete_finished)
    ...
    def _selection_changed(self, *_args: object) -> None:
        ...
        self.delete_button.setEnabled(can_dismiss and bool(item and item.deletion_token)
                                      and can_open and item.recording_id not in active_ids)
        if item is None:
    ...
    def delete_selected(self) -> None:
        ...
        if box.clickedButton() is not remove:
            return
        self._dismiss_pending = True
        self.action_label.setText("파일을 휴지통으로 옮기는 중입니다…")
        self._selection_changed()
        if not self._state.delete_archive_file(item):
            self._on_delete_finished(ArchiveDeleteFinished(error="백엔드에 연결되지 않아 파일을 삭제하지 못했습니다."))

    def _on_delete_finished(self, result: ArchiveDeleteFinished) -> None:
        self._dismiss_pending = False
        self.action_label.setText(result.error or "파일을 휴지통으로 옮기고 이력에서 제거했습니다.")
        self._selection_changed()
